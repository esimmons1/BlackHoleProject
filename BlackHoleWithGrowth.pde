...
float blackHoleGrowthParticle = .3; // Black hole grows .3 per particle, larger = more growth
float blackHoleGrowthStar = .1; // Black hole grows .1 per star, lower = less growth
PVector blackHole;

// Stars in the background
ArrayList<Star> stars = new ArrayList<Star>();
int numStars = 200;  // Number of stars to spawn initially
...
class Particle {
  ...
  int trailLength = 10;  // Trail length for particles
  ...
  void update() {
    ...
    if (d < eventHorizonRadius) {
      alive = false;
      eventHorizonRadius += blackHoleGrowthParticle; // Added growth rate based on particle
      return;
    }
...
class Star {
  ...
  void update() {
    ...
    if (d < eventHorizonRadius) {
      alive = false;
      eventHorizonRadius += blackHoleGrowthStar;
      return;
    }
